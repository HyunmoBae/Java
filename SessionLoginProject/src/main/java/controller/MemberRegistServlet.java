...
public class MemberRegistServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//비즈니스 로직을 호출할때 클라이언트에서 파라미터로 전송된 데이터들은 하나씩 인자로 전달하지 않는다.
		//DTO(Data Transform Object) : 클라이언트에서 전송된 파라미터 값들을 속성값으로 저장해서 모델의 비즈니스 메소드로 인자를 전달하는 용도, setter, getter 정의됨
		//VO(Value Object) : 주로 데이터베이스에서 조회한 레코드 데이터들을 객체로 변환. getter만 정의, 그렇지만 최근에는 혼용해서 쓵다. 
